...
- **按“型号 → 版本号 → 序列号”管理板卡**：先选择板卡型号，再查找其版本号和具体 PCB 序列号。已有版本可下拉选择，也能在新建或编辑板卡时填写新版本；序列号用于区分实物板。每块板还可记录状态和备注，并查看问题、工作记录与 Cell 组成的时间线。下拉候选按需加载，不会一次读取全部板卡。
...
1. **建立板卡档案**：依次填写板卡型号、版本号和唯一的 PCB 序列号；需要统筹多块板时，再创建项目并勾选关联板卡。
